...
def image_inference():
    ...
    try:
        ...
        image_test = cv2.imdecode(
            np.fromstring(image_file_test.read(), np.uint8), cv2.IMREAD_COLOR
        )
        ...
        db.save_model_output_to_db(
            username, str(result), str(execution_time)
        )  # DataBase
...
def get_user_stats():
    ...
    for row in result:
        stats.append(
            {"timestamp": row[0], "model_output": row[1], "execution_time": row[2]}
        )
